...
class AdditiveAttention(nn.Module):
    ...
    def forward(self, lstm_outputs, final_state):
        query_with_time = final_state.unsqueeze(1) 
        score = self.V(torch.tanh(self.W1(lstm_outputs) + self.W2(query_with_time)))
        attention_weights = F.softmax(score, dim=1) 
        context_vector = torch.sum(attention_weights * lstm_outputs, dim=1)
        return context_vector, attention_weights

# ================= 2. 主模型：CNN + BiLSTM + Attention =================
class SimpleCNN(nn.Module):
    def __init__(self, feature_dim=256):
        super(SimpleCNN, self).__init__()
        # 典型的 3 层 CNN 结构提取空间特征
...
class CNNBiLSTMAttention(nn.Module):
    def __init__(self, num_classes=3, lstm_units=128):
        ...
        self.cnn = SimpleCNN(feature_dim=self.feature_dim)
    ...
    def forward(self, x):
        ...
        # 融合 batch 和 seq_len 维度送入 CNN
        x = x.view(batch_size * seq_len, 1, h, w)
        features = self.cnn(x) 
        
        # 恢复成 (batch_size, seq_len, feature_dim) 送入 BiLSTM
        features = features.view(batch_size, seq_len, self.feature_dim) 
        
        # BiLSTM 的输出包含 outputs 和状态 (h_n, c_n)
        lstm_out, (h_n, c_n) = self.lstm(features)
        
        # 提取最后一层的前向与后向隐藏状态 h_n 作为 Attention 的 Query
...
def main():
    ...
    # 替换为新的 CNN + BiLSTM + Attention 模型
    model = CNNBiLSTMAttention(num_classes=3).to(device)
    ...
    try:
        ...
        if os.path.exists("best_cnn_bilstm.pth"):
            model.load_state_dict(torch.load("best_cnn_bilstm.pth", map_location=device))
