...
class ResNetLSTMAttention(nn.Module):
    def __init__(self, num_classes=3, lstm_units=128):
        ...
        # 注意力层输入维度改为 lstm_units (单向)
        self.attention = AdditiveAttention(hidden_dim=lstm_units)
        
        # --- 分类器 ---
        # 线性层输入维度改为 lstm_units
    ...
    def forward(self, x):
        ...
        # lstm_out: [batch, seq_len, lstm_units]
        # h_n: [1, batch, lstm_units] (因为是单层单向)
        lstm_out, (h_n, c_n) = self.lstm(features)
        
        # 单向 LSTM 直接取 h_n[0] 作为最终隐藏状态 (不需要拼接)
        state_h = h_n[0] 
...
def main():
    ...
    # 实例化 LSTM 模型 (修改点)
    model = ResNetLSTMAttention(num_classes=3).to(device)
    ...
    try:
        for epoch in range(epochs):
            ...
            if current_val_acc > best_val_acc:
                ...
                # 修改了保存的模型名称
                torch.save(model.state_dict(), "best_ResNet_lstm.pth")
        ...
        # 对应修改加载的模型名称
        if os.path.exists("best_ResNet_lstm.pth"):
            model.load_state_dict(torch.load("best_ResNet_lstm.pth", map_location=device))
