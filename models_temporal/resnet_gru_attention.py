...
class ResNetGRUAttention(nn.Module):
    def __init__(self, num_classes=3, gru_units=128):
        ...
        # --- 时间序列提取：GRU (修改：bidirectional=False) ---
        self.gru = nn.GRU(
        ...
        # --- 注意力层 (修改：维度从 2*gru_units 改为 gru_units) ---
        self.attention = AdditiveAttention(hidden_dim=gru_units)
        
        # --- 分类器 (修改：输入维度改为 gru_units) ---
        self.classifier = nn.Sequential(
            nn.Linear(gru_units, 256),
    ...
    def forward(self, x):
        ...
        # gru_out: [batch, seq_len, gru_units]
        # h_n: [num_layers, batch, gru_units]
        gru_out, h_n = self.gru(features)
        
        # 提取最后一个隐状态 (单向 GRU 直接取 h_n 的最后一层)
        state_h = h_n[-1] 
...
def main():
    # --- 开启日志记录 ---
    log_file = f"ResNet_gru_attention_log_{datetime.now().strftime('%m%d_%H%M')}.txt"
    ...
    # 模型实例化 (改为单向 GRU 模型)
    model = ResNetGRUAttention(num_classes=3).to(device)
    optimizer = torch.optim.Adam(model.parameters(), lr=lr)
    criterion = nn.CrossEntropyLoss()
    scaler = torch.cuda.amp.GradScaler() 

    best_val_acc = 0.0
    save_path = "best_ResNet_gru.pth"
